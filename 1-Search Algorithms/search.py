...
# Code submitted/amended by Huzefa S.H. Kagalwala (C48290423)
...
#############################################################################
    ...
    while len(open_set) != 0:
        ...
        for j in range(len(ACTIONS)):
            if ((current_node[0] + ACTIONS[j][0]) < 0 or (current_node[1] + ACTIONS[j][1]) < 0 or(current_node[0] + ACTIONS[j][0]) > (n_rows-1) or (current_node[1] + ACTIONS[j][1]) > (n_cols-1)):
                continue
            if (current_node[0] + ACTIONS[j][0], current_node[1] + ACTIONS[j][1]) in obstacles:
                continue
            child = (current_node[0] + ACTIONS[j][0], current_node[1] + ACTIONS[j][1])
            if child not in open_set and child not in closed_set:
                if child == goal:
                    parent[child[0]][child[1]] = current_node[0], current_node[1]
                    current_node = goal
                    actions[child[0]][child[1]] = ACTIONS[j][0], ACTIONS[j][1]
                    break
                else:
                    open_set.add(child)
                    parent[child[0]][child[1]] = current_node[0], current_node[1]
...
def uniform_cost_search(grid_size, start, goal, obstacles, costFn, logger):
    ...
    # ----------------------------------------
#############################################################################
    open_set.put(start, costFn((start[0],start[1])))
    while len(open_set) != 0:
        current_node, current_cost = open_set.pop()
        if current_node == goal:
            break
        closed_set.add(current_node)
        for j in range(len(ACTIONS)):
            ...
            if child in closed_set:
                ...
                if child not in open_set:
                    ...
                    parent[child[0]][child[1]] = current_node[0], current_node[1]
                    actions[child[0]][child[1]] = ACTIONS[j][0], ACTIONS[j][1]
                elif child in open_set and child_cost < open_set.get(child):
...
#############################################################################
    ...
    open_set.put(start, costFn(start)+heuristic(start,start))
    while len(open_set) != 0:
        current_node, f_cost = open_set.pop()
        g_cost = f_cost - heuristic(goal,current_node)
        if current_node == goal:
            #closed_set.add(current_node)
            break
        closed_set.add(current_node)
        for j in range(len(ACTIONS)):
            ...
            if (current_node[0] + ACTIONS[j][0], current_node[1] + ACTIONS[j][1]) in obstacles:
                continue
            child = (current_node[0] + ACTIONS[j][0], current_node[1] + ACTIONS[j][1])
            h = heuristic(goal,child)
            g = g_cost + costFn(child)
            child_cost = g + h
            if child in closed_set:
                continue
            else:
